...
- Added `printer/controller.ts` defining a `PrinterController` contract with a no-op default so interpreter logic can delegate printing without owning transport.
