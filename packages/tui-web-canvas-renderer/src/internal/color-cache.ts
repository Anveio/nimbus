...
const HEX_REGEX = /^#(?:([\da-fA-F]{2})([\da-fA-F]{2})([\da-fA-F]{2})([\da-fA-F]{2})?|([\da-fA-F])([\da-fA-F])([\da-fA-F]))$/
...
const parseHex = (input: string): ColorTuple | null => {
  ...
  if (match[1] && match[2] && match[3]) {
    ...
    return [normaliseByte(r), normaliseByte(g), normaliseByte(b), normaliseByte(a)]
  }
  if (match[5] && match[6] && match[7]) {
    const r = parseInt(match[5]!, 16)
    const g = parseInt(match[6]!, 16)
    const b = parseInt(match[7]!, 16)
    return [normaliseByte(r * 17), normaliseByte(g * 17), normaliseByte(b * 17), 1]
  }
  return null
}
...
const parseRgba = (input: string): ColorTuple | null => {
  ...
  return [normaliseByte(r), normaliseByte(g), normaliseByte(b), normaliseAlpha(a)]
}
