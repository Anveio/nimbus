...
const makeCell = (char: string): TerminalCell => ({
  char,
  attr: createAttributes(),
  protected: false,
})

const cellOrBlank = (cell: TerminalCell | null | undefined): TerminalCell =>
  cell ? cloneCell(cell) : makeCell(' ')
...
test.describe('createCanvasRenderer (browser)', () => {
  ...
  test('insert characters shifts reuse GPU buffers', async ({ page }) => {
    ...
    await withHarness(page, async () => {
      const theme = createTheme()
      const snapshot = createSnapshot(1, 5)
      snapshot.buffer[0]![0] = makeCell('A')
      snapshot.buffer[0]![1] = makeCell('B')
      snapshot.buffer[0]![2] = makeCell('C')
      snapshot.buffer[0]![3] = makeCell('D')
      ...
      try {
        ...
        test.skip(true, `GPU renderer failed to initialise: ${(error as Error).message}`)
      }
      ...
      const blankCell = makeCell(' ')
      nextSnapshot.buffer[0]![4] = snapshot.buffer[0]![3] ? cloneCell(snapshot.buffer[0]![3]!) : blankCell
      nextSnapshot.buffer[0]![3] = snapshot.buffer[0]![2] ? cloneCell(snapshot.buffer[0]![2]!) : blankCell
      nextSnapshot.buffer[0]![2] = snapshot.buffer[0]![1] ? cloneCell(snapshot.buffer[0]![1]!) : blankCell
      nextSnapshot.buffer[0]![1] = snapshot.buffer[0]![0] ? cloneCell(snapshot.buffer[0]![0]!) : blankCell
      nextSnapshot.buffer[0]![0] = makeCell('X')
      ...
      const updates: TerminalUpdate[] = [
        {
          ...
          cells: Array.from({ length: 5 }, (_, column) => ({
            row: 0,
            column,
            cell: nextSnapshot.buffer[0]![column]!,
          })),
        },
      ]
      ...
      const after = await getDiagnostics(page)
      expect(after).toBeTruthy()
      expect(after?.gpuCellsProcessed).toBe(1)
      if (after?.gpuDirtyRegionCoverage !== null) {
        expect(after?.gpuDirtyRegionCoverage).toBeCloseTo(1 / snapshot.columns)
      }
      if (
        typeof after?.gpuBytesUploaded === 'number' &&
        typeof initialDiagnostics?.gpuBytesUploaded === 'number'
  ...
  test('delete characters shifts reuse GPU buffers', async ({ page }) => {
    ...
    await withHarness(page, async () => {
      ...
      snapshot.buffer[0]![2] = makeCell('C')
      snapshot.buffer[0]![3] = makeCell('D')
      snapshot.buffer[0]![4] = makeCell('E')
      ...
      const nextSnapshot = structuredClone(snapshot)
      nextSnapshot.buffer[0]![0] = cellOrBlank(snapshot.buffer[0]![1])
      nextSnapshot.buffer[0]![1] = cellOrBlank(snapshot.buffer[0]![2])
      nextSnapshot.buffer[0]![2] = cellOrBlank(snapshot.buffer[0]![3])
      nextSnapshot.buffer[0]![3] = cellOrBlank(snapshot.buffer[0]![4])
      nextSnapshot.buffer[0]![4] = makeCell(' ')
  ...
  test('insert before double-width glyph preserves geometry', async ({
    ...
    await withHarness(page, async () => {
      ...
      const snapshot = createSnapshot(1, 5)
      snapshot.buffer[0]![0] = makeCell('A')
      snapshot.buffer[0]![1] = makeCell('漢')
      snapshot.buffer[0]![2] = makeCell(' ') // trailing cell of double width
      snapshot.buffer[0]![3] = makeCell('B')
      ...
      const nextSnapshot = structuredClone(snapshot)
      nextSnapshot.buffer[0]![4] = cellOrBlank(snapshot.buffer[0]![3])
      nextSnapshot.buffer[0]![3] = cellOrBlank(snapshot.buffer[0]![2])
      nextSnapshot.buffer[0]![2] = cellOrBlank(snapshot.buffer[0]![1])
      nextSnapshot.buffer[0]![1] = makeCell('漢')
      nextSnapshot.buffer[0]![0] = makeCell('X')
  ...
  test.fixme(true, 'Double-height row insert heuristics pending implementation')
  test('double-height row scroll keeps paired geometry aligned', async ({
    page,
  }) => {
    const supportsWebgl = await isWebglSupported(page)
    test.skip(!supportsWebgl, 'WebGL not supported in this environment')
    ...
    await withHarness(page, async () => {
      ...
      snapshot.lineAttributes[0] = 'double-top'
      snapshot.lineAttributes[1] = 'double-bottom'
      snapshot.buffer[0]![0] = makeCell('A')
      snapshot.buffer[0]![1] = makeCell('B')
      snapshot.buffer[0]![2] = makeCell('C')
      snapshot.buffer[0]![3] = makeCell('D')

      snapshot.buffer[1]![0] = makeCell('A')
      snapshot.buffer[1]![1] = makeCell('B')
      snapshot.buffer[1]![2] = makeCell('C')
      snapshot.buffer[1]![3] = makeCell('D')
      ...
      expect(after.gpuCellsProcessed).toBeGreaterThan(0)
    })
  })
})
